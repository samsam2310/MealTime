...
def getDatabaseFromEnv(is_log_mode = False):
    ...
    mongo_client =
    	MongoClient(host=DB_HOST, replicaset=DB_REPLSET,
...
class Table():
	...
	_ORM_mongo_client = getDatabaseFromEnv(True)
	...
	@classmethod
	def upsert(cls, orm_object, query_keys):
		cls._checkInstance('orm_object must be a isinstance of %s' % cls)
		# TODO

	@classmethod
	def getCollectionName(cls):
		return cls._ORM_collection_name
